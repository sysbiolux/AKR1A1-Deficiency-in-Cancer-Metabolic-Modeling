function fittingcurve_TS(signal) % get the densities
% Create right curve
figure
hold on
[probability_estimate,xi] = ksdensity(signal);
plot(xi,probability_estimate,'-k','LineWidth',2);
ylabel('Density'); xlabel('expression')
...
if numel(hybrid)> numel(probability_estimate)
    ...
    x = zeros(numel(probability_estimate),1); % create new x-axis: WHY???
    x=xi;
else
...
title({'Density plot with hybrid curves',['Sample: xxx']},'interpreter','none');

%% flux_sampling_analysis.m
% Unified legacy reaction-level Flux Sampling analysis.
%
% Modes:
%   standard1500 - RAW 769-P/Huh7 workflow, 1500 samples
%   onoff1000    - TCGA ON/OFF workflow, 1000 samples
%
% Reproducibility rule:
% The original FBA, FVA, mapping, statistics, directionality, FC, SNR,
% filtering, sorting, table construction and pathway analysis are retained.
% Only paths, input filenames, sample count, comparison calls and historical
% output filename tags are selected by datasetType.
%
% MATLAB compatibility: R2019.
%
% Expected relative layout when MATLAB starts in scripts/:
%   ../data/reference/consistent_model.mat
%   ../data/sampling/standard/
%   ../data/sampling/onoff/
%   ../data/flux_sampling/standard/
%   ../data/flux_sampling/onoff/
...
referenceModelFile = '../data/reference/consistent_model.mat';

if strcmp(datasetType,'standard1500')
    inputFolder = '../data/sampling/standard';
    outputFolder = '../data/flux_sampling/standard';
    nSamples = 1500;
    outputModeTag = '';
    outputSuffix = '_1500.xlsx';

    if ~exist(outputFolder,'dir'), mkdir(outputFolder); end

    performAnalysisGSELegacy(1,2:4,'769-P',datasetType,inputFolder,outputFolder,referenceModelFile,nSamples,outputModeTag,outputSuffix);
    performAnalysisGSELegacy(5,6:8,'Huh7',datasetType,inputFolder,outputFolder,referenceModelFile,nSamples,outputModeTag,outputSuffix);

elseif strcmp(datasetType,'onoff1000')
    inputFolder = '../data/sampling/onoff';
    outputFolder = '../data/flux_sampling/onoff';
    nSamples = 1000;
    outputModeTag = 'onoff';
    outputSuffix = '.xlsx';
    ...
    performAnalysisGSELegacy(1,2,'KIRC-ONOFF',datasetType,inputFolder,outputFolder,referenceModelFile,nSamples,outputModeTag,outputSuffix);
    performAnalysisGSELegacy(3,4,'KIRP-ONOFF',datasetType,inputFolder,outputFolder,referenceModelFile,nSamples,outputModeTag,outputSuffix);
    performAnalysisGSELegacy(5,6,'KICH-ONOFF',datasetType,inputFolder,outputFolder,referenceModelFile,nSamples,outputModeTag,outputSuffix);
    performAnalysisGSELegacy(7,8,'LIHC-ONOFF',datasetType,inputFolder,outputFolder,referenceModelFile,nSamples,outputModeTag,outputSuffix);

else
    error('datasetType must be ''standard1500'' or ''onoff1000''.');
end

function performAnalysisGSELegacy(ctrl,treatments,model_prefix_str,datasetType,inputFolder,outputFolder,referenceModelFile,nSamples,outputModeTag,outputSuffix)
...
for i = 1:numel(ctrl)
    for j = 1:numel(treatments)
        ...
        for k = 1:numel(treatmentNames)
            ...
            if treatmentName == "medium"
                disp (treatmentName)
                if strcmp(datasetType,'standard1500')
                    fileA_medium = fullfile(inputFolder,['SamplingResults_medium_1500_model_' num2str(ctrl(i)) '.mat']);
                else
                    fileA_medium = fullfile(inputFolder,['samplingResults_mediumonoff_1000_model_' num2str(ctrl(i)) '.mat']);
                end
                if strcmp(datasetType,'standard1500')
                    fileB_medium = fullfile(inputFolder,['SamplingResults_medium_1500_model_' num2str(treatments(j)) '.mat']);
                else
                    fileB_medium = fullfile(inputFolder,['samplingResults_mediumonoff_1000_model_' num2str(treatments(j)) '.mat']);
                end
                % Load data
                load(fileA_medium, 'x');
                model1_medium = x.modelSampling;
                data1_medium = x.samples(:, 1:nSamples);
                load(fileB_medium, 'x');
                model2_medium = x.modelSampling;
                data2_medium = x.samples(:, 1:nSamples);
            ...
            elseif treatmentName == "KO_GLO1"
                disp (treatmentName)
                if strcmp(datasetType,'standard1500')
                    fileA_KO = fullfile(inputFolder,['samplingResults_medium_' treatmentName '_1500_model_' num2str(ctrl(i)) '.mat']);
                else
                    fileA_KO = fullfile(inputFolder,['samplingResults_medium_' treatmentName 'onoff_1000_model_' num2str(ctrl(i)) '.mat']);
                end
                if strcmp(datasetType,'standard1500')
                    fileB_KO = fullfile(inputFolder,['samplingResults_medium_' treatmentName '_1500_model_' num2str(treatments(j)) '.mat']);
                else
                    fileB_KO = fullfile(inputFolder,['samplingResults_medium_' treatmentName 'onoff_1000_model_' num2str(treatments(j)) '.mat']);
                end
                % Load data
                load(fileA_KO, 'x');
                model1_KO = x.modelSampling;
                data1_KO = x.samples(:, 1:nSamples);
                load(fileB_KO, 'x');
                model2_KO = x.modelSampling;
                data2_KO = x.samples(:, 1:nSamples);
        ...
        file_fba_medium=fullfile(outputFolder,[model_prefix_str '_fba_medium' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
        deleteLegacyOutput(file_fba_medium,datasetType)
        ...
        file_fba_KO=fullfile(outputFolder,[model_prefix_str '_fba_KO' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
        deleteLegacyOutput(file_fba_KO,datasetType)
    ...
    resA_sampling_medium=zeros(numel(model_orig.rxns),nSamples);
...
%     
    ...
    resB_sampling_medium=zeros(numel(model_orig.rxns),nSamples);
    resB_sampling_medium(IA2m,:)=data2_medium;%data2
    
    resB_sampling_KO=zeros(numel(model_orig.rxns),nSamples);
    ...
    if strcmp(datasetType,'standard1500')
        ...
        direction_change_ko = zeros(10600, 1);
    end
    for counter=1:size(resA_sampling_medium,1)
    ...
    file_medium=fullfile(outputFolder,[model_prefix_str '_FluxSamplingStatst_all_medium_5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file_medium,datasetType)
    ...
    file=fullfile(outputFolder,[model_prefix_str '_FluxSamplingStatsallrank_medium_5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    ...
    file_KO=fullfile(outputFolder,[model_prefix_str '_FluxSamplingStatst_all_KO_5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file_KO,datasetType)
    ...
    file=fullfile(outputFolder,[model_prefix_str '_FluxSamplingStatsallrank_KO_5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    ...
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Glyco_medium5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    ...
    upt=sortrows(upt,7,'descend') %by SNR
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Glyco_KO5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    %%
    %Pyruvate metabolism
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Pyr_medium5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Pyr_KO5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_PPP_medium5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_PPP_KO5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Glutath_medium5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Glutath_KO5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Chol_medium5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Chol_KO5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_ROS_medium5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_ROS_KO5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    %%
    %TCA pathway
    ...
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_TCA_medium5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_TCA_KO5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    %%
    ...
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Nucleot_medium5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=fullfile(outputFolder,[model_prefix_str '_fluxSamplingStats_a_Nucleot_KO5' outputModeTag num2str(ctrl(i)) '_vs_' num2str(treatments(j)) outputSuffix]);
    deleteLegacyOutput(file,datasetType)
    writetable(upt,file,'WriteRowNames',true)
    end
end
...
function deleteLegacyOutput(file,datasetType)
% Preserve the historical deletion behavior for each source workflow.
if strcmp(datasetType,'standard1500')

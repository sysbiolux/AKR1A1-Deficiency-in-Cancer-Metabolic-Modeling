...
% For TCGA ON/OFF, there can be more than 30 samples, so j = 31 fails.
if figflag
    figure; hold on

    nSamples = length(colnames);
    nRows = ceil(sqrt(nSamples));
    nCols = ceil(nSamples/nRows);
    ...
    saveas(gcf,['Figures\Discretization\density_subplots.png']);
    close all
end
%% FluxSampling_TCGAdata.m
% FluxSampling_TCGAdata ? flux sampling statistics from TCGA data
% Run flux sampling analysis for TCGA ON/OFF models.
%
...
%   samplingResults_mediumonoff_1000_model_1.mat ... model_8.mat
%   samplingResults_medium_KO_GLO1onoff_1000_model_1.mat ... model_8.mat
...
% Run flux sampling analysis for each TCGA cancer type.
performAnalysisGSEONOFF(1,2,'KIRC-ONOFF');
performAnalysisGSEONOFF(3,4,'KIRP-ONOFF');
performAnalysisGSEONOFF(5,6,'KICH-ONOFF');
performAnalysisGSEONOFF(7,8,'LIHC-ONOFF');


function performAnalysisGSEONOFF(ctrl,treatments,model_prefix_str)

%% Perform TCGA flux sampling comparison analysis
% Inputs:
%   ctrl             - control model index
%   treatments       - AKR1A1-deficient model index
%   model_prefix_str - TCGA cancer type identifier
%
% Steps:
%   1. Load sampled metabolic models.
%   2. Analyze medium and KO_GLO1 conditions.
%   3. Perform FBA and FVA.
%   4. Map fluxes to the reference model.
%   5. Calculate flux statistics.
%   6. Export reaction-level tables.
...
for i = 1:numel(ctrl)
    for j = 1:numel(treatments)
        ...
        for k = 1:numel(treatmentNames)
            ...
            if treatmentName == "medium"
                disp (treatmentName)
                fileA_medium = ['samplingResults_mediumonoff_1000_model_' num2str(ctrl(i)) '.mat'];
                fileB_medium = ['samplingResults_mediumonoff_1000_model_' num2str(treatments(j)) '.mat'];
                % Load data
                load(fileA_medium, 'x');
                model1_medium = x.modelSampling;
                data1_medium = x.samples(:, 1:1000);
                load(fileB_medium, 'x');
                model2_medium = x.modelSampling;
                data2_medium = x.samples(:, 1:1000);
            ...
            elseif treatmentName == "KO_GLO1"
                disp (treatmentName)
                fileA_KO = ['samplingResults_medium_' treatmentName 'onoff_1000_model_' num2str(ctrl(i)) '.mat'];
                fileB_KO = ['samplingResults_medium_' treatmentName 'onoff_1000_model_' num2str(treatments(j)) '.mat'];
                % Load data
                load(fileA_KO, 'x');
                model1_KO = x.modelSampling;
                data1_KO = x.samples(:, 1:1000);
                load(fileB_KO, 'x');
                model2_KO = x.modelSampling;
                data2_KO = x.samples(:, 1:1000);
        ...
        file_fba_medium=[model_prefix_str '_fba_medium' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
        delete(file_fba_medium)
        ...
        file_fba_KO=[model_prefix_str '_fba_KO' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
        delete(file_fba_KO)
    ...
    resA_sampling_medium=zeros(numel(model_orig.rxns),1000);
...
%     
    ...
    resB_sampling_medium=zeros(numel(model_orig.rxns),1000);
    resB_sampling_medium(IA2m,:)=data2_medium;%data2
    
    resB_sampling_KO=zeros(numel(model_orig.rxns),1000);
    ...
    file_medium=[model_prefix_str '_FluxSamplingStatst_all_medium_5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file_medium)
    ...
    file=[model_prefix_str '_FluxSamplingStatsallrank_medium_5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    ...
    file_KO=[model_prefix_str '_FluxSamplingStatst_all_KO_5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file_KO)
    ...
    file=[model_prefix_str '_FluxSamplingStatsallrank_KO_5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    ...
    file=[model_prefix_str '_fluxSamplingStats_a_Glyco_medium5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    ...
    upt=sortrows(upt,7,'descend') %by SNR
    file=[model_prefix_str '_fluxSamplingStats_a_Glyco_KO5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    %%
    %Pyruvate metabolism
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=[model_prefix_str '_fluxSamplingStats_a_Pyr_medium5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=[model_prefix_str '_fluxSamplingStats_a_Pyr_KO5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    file=[model_prefix_str '_fluxSamplingStats_a_PPP_medium5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=[model_prefix_str '_fluxSamplingStats_a_PPP_KO5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    file=[model_prefix_str '_fluxSamplingStats_a_Glutath_medium5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=[model_prefix_str '_fluxSamplingStats_a_Glutath_KO5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    file=[model_prefix_str '_fluxSamplingStats_a_Chol_medium5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=[model_prefix_str '_fluxSamplingStats_a_Chol_KO5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    file=[model_prefix_str '_fluxSamplingStats_a_ROS_medium5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=[model_prefix_str '_fluxSamplingStats_a_ROS_KO5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    %%
    %TCA pathway
    ...
    file=[model_prefix_str '_fluxSamplingStats_a_TCA_medium5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=[model_prefix_str '_fluxSamplingStats_a_TCA_KO5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    %%
    ...
    file=[model_prefix_str '_fluxSamplingStats_a_Nucleot_medium5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    ...
    upt.formula=modelformula
    upt=sortrows(upt,7,'descend') %by SNR
    file=[model_prefix_str '_fluxSamplingStats_a_Nucleot_KO5onoff' num2str(ctrl(i)) '_vs_' num2str(treatments(j)) '.xlsx'];
    delete(file)
    writetable(upt,file,'WriteRowNames',true)
    end
end
